...
var JOSEError, JWTClaimValidationFailed, JWTExpired, JOSEAlgNotAllowed, JOSENotSupported, JWEDecryptionFailed, JWEInvalid, JWSInvalid, JWTInvalid, JWKSInvalid, JWKSNoMatchingKey, JWKSMultipleMatchingKeys, JWKSTimeout, JWSSignatureVerificationFailed;
...
function createAuthCache(config) {
  ...
  async function decrypt3(encrypted) {
    ...
    if (!AUTH_SECRET_PREVIOUS)
      return null;
    try {
      const previousKey = await deriveKeyFromSecret(AUTH_SECRET_PREVIOUS, salt, "encryption-key");
